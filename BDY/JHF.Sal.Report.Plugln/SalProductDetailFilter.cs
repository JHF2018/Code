...
{
  public   class SalProductDetailFilter : AbstractCommonFilterPlugIn
    {

        private List<long> lstSalOrg = new List<long>();
        private List<long> _lstOrgs;
        ...
        private void CheckValit(ButtonClickEventArgs e)
        {
            ...
            {
                e.Cancel = true;
                this.View.ShowErrMessage(stringBuilder.ToString().Trim(), ResManager.LoadKDString("日期范围错误", "004104030002449", SubSystemType.SCM, new object[0]), MessageBoxType.Advise);
            }
        }
        ...
        private void GetSaleOrderListData(BeforeF7SelectEventArgs e, string key_Field, string listName, string selectColumn)
        {
            if (listName == null || listName.ToString().Length == 0)
            {
                return;
            }
            ...
            listShowParameter.FormId = listName.ToString();
